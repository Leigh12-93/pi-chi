...
export function BranchMenu({ owner, repo, currentBranch = 'main', onSwitch }: BranchMenuProps) {
  ...
  const handleCreateBranch = async () => {
    ...
    } catch (e) { console.warn('[pi:branch] Failed to create branch:', e) }
    setCreating(false)
  }
  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-pi-text-dim hover:text-pi-text bg-pi-surface border border-pi-border rounded-lg transition-colors"
      >
      ...
      {open && (
        <div className="absolute top-full mt-1 left-0 w-64 bg-pi-bg border border-pi-border rounded-xl shadow-xl z-50 overflow-hidden">
          <div className="p-2 border-b border-pi-border">
            <div className="flex gap-1.5">
              <input
                ...
                className="flex-1 px-2 py-1 text-xs bg-pi-surface border border-pi-border rounded text-pi-text placeholder:text-pi-text-dim/50 focus:outline-none focus:border-pi-accent"
                onKeyDown={e => e.key === 'Enter' && handleCreateBranch()}
              />
              <button
                onClick={handleCreateBranch}
                disabled={!newBranchName.trim() || creating}
                className="px-2 py-1 text-xs bg-pi-accent text-white rounded hover:bg-pi-accent-hover disabled:opacity-50 transition-colors"
              >
          ...
          <div className="max-h-48 overflow-y-auto">
            {loading ? (
              ...
              <p className="text-xs text-pi-text-dim text-center py-3">No branches found</p>
            ) : (
              branches.map(branch => (
                <button
                  key={branch}
                  ...
                  className={cn(
                    'w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-pi-surface transition-colors',
                    branch === currentBranch ? 'text-pi-accent' : 'text-pi-text-dim',
                  )}
                >
