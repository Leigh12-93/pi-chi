...
export function ChangeSummary({ changes, onFileClick }: ChangeSummaryProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-xl border border-pi-border/60 bg-pi-surface/40 overflow-hidden"
    >
      {/* Collapsed header / toggle */}
      <button
        onClick={() => setExpanded(prev => !prev)}
        className="flex items-center gap-2 w-full px-3 py-2 text-left hover:bg-pi-surface-hover/40 transition-colors group"
      >
        <div className="w-5 h-5 rounded-md bg-pi-accent/10 border border-pi-accent/20 flex items-center justify-center shrink-0">
          <FileCode className="w-3 h-3 text-pi-accent" />
        </div>
        <span className="text-[12px] text-pi-text font-medium flex-1 min-w-0">
          {totalCount} file{totalCount !== 1 ? 's' : ''} changed
        </span>
        <span className="text-[11px] text-pi-text-dim/60 shrink-0 hidden sm:inline">
          {summaryParts.join(', ')}
        </span>
        <motion.div
          ...
          className="shrink-0"
        >
          <ChevronDown className="w-3.5 h-3.5 text-pi-text-dim/40 group-hover:text-pi-text-dim transition-colors" />
        </motion.div>
      </button>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      type === 'deleted'
                        ...
                        : 'hover:bg-pi-bg/60 cursor-pointer',
                    )}
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        type === 'deleted'
                          ? 'text-pi-text-dim/50 line-through'
                          : 'text-pi-text-dim',
                      )}
                      title={path}
                    >
                    ...
                    <span className="text-[10px] text-pi-text-dim/30 shrink-0 hidden sm:inline font-mono">
