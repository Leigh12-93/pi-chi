...
export function EnvVarInputCard({
  ...
  return (
    <div className="border border-forge-border bg-forge-surface/30 rounded-xl p-3.5 text-[12px]">
      <div className="flex items-center gap-2 mb-2.5">
        <div className="w-5 h-5 rounded-md flex items-center justify-center shrink-0 text-forge-accent bg-forge-accent/10">
          <Key className="w-3 h-3" />
        </div>
        <span className="font-medium text-forge-text">Environment Variables Required</span>
      </div>
      <div className="space-y-2.5">
        {variables.map((v) => (
          <div key={v.name}>
            <div className="flex items-center gap-1 mb-1">
              <code className="text-[11px] font-mono text-forge-text font-medium">{v.name}</code>
              {v.required !== false && <span className="text-red-500 text-[9px]">*</span>}
            </div>
            {v.description && (
              <p className="text-[11px] text-forge-text-dim mb-1">{v.description}</p>
            )}
            <input
              ...
              placeholder={v.name}
              className="w-full px-2.5 py-1.5 rounded-md bg-forge-bg border border-forge-border text-[11.5px] font-mono text-forge-text placeholder:text-forge-text-dim/40 focus:outline-none focus:border-forge-accent/40 focus:shadow-[0_0_0_3px_var(--color-forge-ring)] transition-all"
            />
          </div>
        ))}
      </div>
