...
function FindingRow({ finding, decision, onDecide, expanded, onToggle }: {
  ...
  return (
    <div className={cn('rounded-lg border transition-all', decision === 'fix' ? 'border-emerald-500/20 bg-emerald-950/10' : decision === 'leave' ? 'border-pi-border/10 opacity-50' : config.border, config.bg)}>
      <div className="flex items-center gap-2.5 px-3 py-2">
        <Icon className={cn('w-3.5 h-3.5 shrink-0', config.color)} />
        <button onClick={onToggle} className="flex-1 min-w-0 text-left flex items-center gap-1.5">
          <span className="text-[12px] text-pi-text font-medium truncate">{finding.title}</span>
          <span className="text-[10px] text-pi-text-dim/40 shrink-0 px-1 py-0.5 rounded bg-pi-surface">{finding.category}</span>
          <span className="text-[10px] text-pi-text-dim/30 shrink-0">{EFFORT_LABELS[finding.effort]}</span>
          <ChevronRight className={cn('w-3 h-3 text-pi-text-dim/30 transition-transform shrink-0', expanded && 'rotate-90')} />
        </button>
        {decision === null ? (
          <div className="flex gap-1.5 shrink-0">
            ...
            <button onClick={() => onDecide('leave')} className="px-2 py-1 rounded text-[10px] text-pi-text-dim/50 hover:text-pi-text-dim hover:bg-pi-surface transition-colors">Leave</button>
          </div>
        ) : decision === 'fix' ? (
          ...
          <span className="text-[10px] text-pi-text-dim/30 shrink-0">Skipped</span>
        )}
      </div>
      ...
      <AnimatePresence>
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
            <div className="px-3 pb-2.5 pt-0.5 space-y-2 border-t border-pi-border/10">
              <p className="text-[11.5px] text-pi-text-dim/70 leading-relaxed">{finding.description}</p>
              {finding.currentPattern && (
                <div>
                  <div className="text-[10px] text-pi-text-dim/40 uppercase tracking-wide mb-0.5">Current</div>
              ...
              {finding.suggestedPattern && (
                <div>
                  <div className="text-[10px] text-pi-text-dim/40 uppercase tracking-wide mb-0.5">Suggested</div>
              ...
              {finding.file && (
                <div className="text-[10px] text-pi-text-dim/40 font-mono">{finding.file}{finding.affectedFiles && finding.affectedFiles.length > 1 ? ` (+${finding.affectedFiles.length - 1} more)` : ''}</div>
              )}
            </div>
          </motion.div>
        )}
...
export function AuditFindingsCard({ findings: data, onFixSelected, onDismiss }: AuditFindingsCardProps) {
  ...
  return (
    <motion.div
      ...
      className="border border-amber-500/20 rounded-xl overflow-hidden bg-pi-bg/80 backdrop-blur-sm"
    >
      {/* Header */}
      ...
      <div className="px-4 py-3 space-y-3">
        {/* Summary */}
        <p className="text-[12.5px] text-pi-text/80 leading-relaxed">{data.summary}</p>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={handleLeaveAll} className="px-2.5 py-1 rounded text-[10px] text-pi-text-dim/40 hover:text-pi-text-dim hover:bg-pi-surface transition-colors">
            Leave All
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 pt-1">
          {fixCount > 0 ? (
            ...
            <button disabled className="px-4 py-2 rounded-lg text-[12px] font-semibold bg-pi-surface text-pi-text-dim/30 border border-pi-border/20 cursor-not-allowed">
              Select findings to fix
            </button>
          )}
          <button onClick={handleDismiss} className="px-3 py-2 rounded-lg text-[12px] text-pi-text-dim hover:text-pi-text hover:bg-pi-surface transition-all border border-pi-border/30">
            Done — No Fixes Needed
          </button>
          {allDecided && (
            <span className="text-[11px] text-pi-text-dim/40 ml-auto">
