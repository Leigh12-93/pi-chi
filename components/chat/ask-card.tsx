...
export function AskCard({ question, context, options, recommended, allowFreeText = true, onAnswer }: AskCardProps) {
  ...
  return (
    <motion.div
      ...
      className="border border-blue-500/30 rounded-xl overflow-hidden bg-pi-bg/80 backdrop-blur-sm"
    >
      {/* Header */}
      <div className="px-4 py-3 bg-gradient-to-r from-blue-950/40 to-blue-900/20 border-b border-blue-500/20 flex items-center gap-3">
        ...
        <span className="text-[13px] font-semibold text-blue-300">Pi-Chi needs your input</span>
      </div>

      <div className="px-4 py-3 space-y-3">
        {/* Context */}
        {context && (
          <p className="text-[12px] text-pi-text-dim/60 leading-relaxed">{context}</p>
        )}

        {/* Question */}
        <p className="text-[13px] text-pi-text font-medium">{question}</p>
          ...
          <div className="space-y-1.5">
            {options.map(opt => (
              <label key={opt.id} className={cn(
                ...
                selected === opt.id ? 'border-blue-500/40 bg-blue-950/20' : 'border-pi-border/30 hover:border-pi-border/60'
              )}>
                ...
                <div className="flex-1">
                  <div className="text-[12px] text-pi-text font-medium flex items-center gap-2">
                    {opt.label}
                    {opt.id === recommended && (
                  ...
                  {opt.description && <div className="text-[11px] text-pi-text-dim/60 mt-0.5">{opt.description}</div>}
                </div>
              </label>
            ))}
          </div>
        ...
        {allowFreeText && (
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 bg-pi-surface border border-pi-border/30 rounded-lg text-[12px] text-pi-text outline-none resize-none focus:border-blue-500/40 transition-colors"
          />
        )}
        ...
        <button
          ...
          className={cn(
            ...
            (freeText.trim() || selected)
              ...
              : 'bg-pi-surface text-pi-text-dim/30 border-pi-border/20 cursor-not-allowed'
          )}
        >
          Answer
        </button>
