  ...
  if (!githubRepoUrl) {
    ...
    return (
      <div className="p-3 space-y-3">
        <p className="text-xs text-pi-text-dim">No repository connected</p>
        <button
          onClick={() => onAction('create-repo')}
          className="w-full flex items-center gap-2 px-3 py-2 text-xs rounded-lg bg-pi-accent text-white hover:bg-pi-accent/90 active:scale-[0.98] transition-all duration-150"
        >
        ...
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2 text-xs rounded-lg border border-pi-border hover:bg-pi-surface active:scale-[0.98] transition-all duration-150"
        >
          ...
          <div className="space-y-2 animate-fade-in">
            ...
            {loadingRepos ? (
              <div className="flex items-center gap-2 py-3 justify-center">
                ...
                <span className="text-[10px] text-pi-text-dim">Loading repositories...</span>
              </div>
            ) : repos.length > 0 ? (
              <>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-2 py-1.5 text-xs bg-pi-bg border border-pi-border rounded-md focus:outline-none focus:border-pi-accent"
                  />
                </div>
                <div className="max-h-40 overflow-y-auto space-y-0.5 rounded-md border border-pi-border bg-pi-bg">
                  {filteredRepos.slice(0, 20).map(repo => (
                    <button
                      key={repo.full_name}
                      onClick={() => handleConnect(repo.html_url)}
                      disabled={connecting}
                      className="group/repo w-full flex items-center gap-2 px-2 py-1.5 text-xs text-left hover:bg-pi-surface border-l-2 border-l-transparent hover:border-l-pi-accent active:scale-[0.98] transition-all duration-150"
                    >
                      <GitBranch className="w-3 h-3 text-pi-text-dim group-hover/repo:text-pi-accent shrink-0 transition-colors" />
                      <span className="truncate text-pi-text font-mono">{repo.full_name}</span>
                    </button>
                  ))}
                  {filteredRepos.length === 0 && (
                    <p className="text-[10px] text-pi-text-dim text-center py-2">No matching repos</p>
                  )}
                </div>
              </>
            ) : null}
            ...
            <div className="flex items-center gap-2 px-1">
              ...
              <span className="text-[9px] text-pi-text-dim">or enter manually</span>
              <span className="flex-1 h-px bg-pi-border" />
            </div>
            <input
              type="text"
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs rounded-md bg-pi-accent text-white hover:bg-pi-accent/90 disabled:opacity-40 transition-colors"
            >
        ...
        >
          ...
          Push to GitHub
          {changedCount > 0 && (
            <span className="ml-auto px-1.5 py-0.5 text-[10px] font-medium bg-pi-accent/15 text-pi-accent rounded-full">
              {changedCount}
            </span>
          )}
        </button>
  ...
  return (
    <div className="p-3 space-y-3">
      <a
        ...
        className="flex items-center gap-2 text-xs text-pi-accent hover:underline"
      >
      ...
      <button
        ...
        disabled={pulling}
        className="w-full flex items-center gap-2 px-3 py-3 sm:py-2 text-sm sm:text-xs rounded-lg border border-pi-border hover:bg-pi-surface active:scale-[0.98] transition-all duration-150 disabled:opacity-50 min-h-[44px]"
      >
      ...
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-3 sm:py-2 text-sm sm:text-xs rounded-lg border border-pi-border text-pi-text-dim hover:text-red-400 hover:border-red-400/30 hover:bg-red-500/5 active:scale-[0.98] transition-all duration-150 disabled:opacity-50 min-h-[44px]"
      >
