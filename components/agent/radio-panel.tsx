...
const MORNING_ROTATION = ['nova919', 'triplem', 'safm', 'wavelength', 'wavelength']
...
function useIcecastStatus(enabled: boolean): IcecastStatus {
  const [status, setStatus] = useState<IcecastStatus>({
    title: 'Offline', genre: '', listeners: 0, online: false,
  })

  useEffect(() => {
    if (!enabled) return
    const poll = async () => {
      const data = await radioApi('icecast')
...
export function RadioPanel({ fullPage }: RadioPanelProps) {
  ...
  const isPiFrequencyActive = activeStation?.id === 'wavelength' && isPlaying
  const icecast = useIcecastStatus(isPiFrequencyActive)

  // Poll MPD status every 5s
  useEffect(() => {
    const poll = async () => {
      ...
      <div className="h-full flex flex-col bg-pi-bg">
          ...
          <div className="flex items-center gap-3">
            {isPlaying && activeStation && (
              <div className="flex items-center gap-1.5">
                ...
                />
                <span className="text-xs text-pi-text-dim">Live</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto p-6 space-y-6">
            ...
            {morningStation && (
              <div className={cn(
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-pi-text font-medium">
                    {morningStation.name} · {MORNING_START_HOUR}:{String(MORNING_START_MIN).padStart(2, '0')}&ndash;{MORNING_END_HOUR}:{String(MORNING_END_MIN).padStart(2, '0')}
                  </p>
                </div>
                <div className="flex items-center gap-2 text-[10px] text-pi-text-dim font-mono shrink-0">
                  {DAY_NAMES.map((day, i) => {
                    const sid = MORNING_ROTATION[i]
                    const s = STATIONS.find(st => st.id === sid)
                    const isToday = new Date().getDay() - 1 === i
                    return (
                      <span key={day} className={cn(
                        'px-1.5 py-0.5 rounded',
                        isToday ? 'bg-amber-500/20 text-amber-400 font-bold' : ''
                      )}>
                        {day}: {s?.name.split(' ')[0] || sid}
                      </span>
                    )
                  })}
                </div>
                {inMorningWindow && isPlaying && activeStation?.id === morningStation.id && (
                  <span className="text-xs text-amber-400 font-bold shrink-0">AUTO</span>
                )}
              </div>
            )}
              ...
              {STATIONS.map(station => {
                ...
                return (
                  <motion.button
                    ...
                    disabled={loading}
                    className={cn(
                      'rounded-2xl border p-5 text-left transition-all flex flex-col items-center gap-3',
                      loading && 'opacity-60 pointer-events-none',
                      isCurrentlyPlaying
                        ? 'border-pi-accent/40 bg-pi-accent/5 shadow-lg shadow-pi-accent/10 ring-1 ring-pi-accent/20'
                  ...
                  >
                    ...
                    <div className="text-center min-w-0 w-full">
                      <span className="text-sm font-bold text-pi-text block truncate">{station.name}</span>
                      <span className="text-[11px] text-pi-text-dim font-mono">{station.freq}</span>
                      {station.isAI && (
                        <span className="block text-[10px] text-emerald-400 font-medium mt-0.5">AI Generated</span>
                      )}
                    </div>
                    ...
                    {isCurrentlyPlaying ? (
                      ...
                      >
                        ...
                        <span className="text-xs text-pi-accent font-medium">Playing</span>
                      </motion.div>
                    ) : (
                      <div className={cn(
            ...
            {/* Pi Frequency Now Playing */}
            {isPiFrequencyActive && (
              <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                className="rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-500/5 to-cyan-500/5 p-5"
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-pi-text">Pi Frequency</span>
                  </div>
                  <span className="text-[10px] text-pi-text-dim font-mono">
                    {icecast.online ? `${icecast.listeners} listener${icecast.listeners !== 1 ? 's' : ''}` : 'Stream offline'}
                  </span>
                </div>
                {icecast.online && (
                  <div className="space-y-1">
                    <p className="text-sm text-pi-text">
                      <span className="text-pi-text-dim">Now: </span>
                      {icecast.title}
                    </p>
                    {icecast.genre && (
                      <p className="text-xs text-pi-text-dim">
        ...
        {activeStation && (
          <motion.div
            ...
            className="shrink-0 border-t border-pi-border bg-pi-panel/95 backdrop-blur-sm px-6 py-3"
          >
            <div className="max-w-4xl mx-auto flex items-center gap-4">
              {/* Station info */}
              <div className={cn(
                'w-10 h-10 rounded-lg flex items-center justify-center text-xl bg-gradient-to-br shrink-0',
                activeStation.color
              )}>
                {activeStation.logo}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-pi-text font-medium truncate">{activeStation.name}</p>
                <p className="text-[10px] text-pi-text-dim">
                  {activeStation.freq} · Pi speaker
                  {isPiFrequencyActive && icecast.online && ` · ${icecast.title}`}
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-3 rounded-full bg-pi-accent text-white hover:bg-pi-accent-hover transition-all"
                >
              ...
              <div className="flex items-center gap-2 w-32 shrink-0">
                <button
                  onClick={toggleMute}
                  className="p-1.5 rounded text-pi-text-dim hover:text-pi-text transition-all"
                >
                ...
                <input
                  ...
                  className="flex-1 h-1.5 accent-pi-accent cursor-pointer"
                />
              </div>
            </div>
          </motion.div>
  ...
  /* ─── Compact layout (legacy/embedded) ─────────── */
