...
export function FindReplacePanel({ open, onClose, files, onReplace, activeFile }: FindReplacePanelProps) {
  ...
  return (
    <div className="border-b border-pi-border bg-pi-panel px-3 py-2 space-y-2">
      <div className="flex items-center gap-2">
        <div className="flex-1 flex items-center gap-1.5 bg-pi-surface border border-pi-border rounded-lg px-2">
          ...
          <button
            ...
            className={cn('p-0.5 rounded', caseSensitive ? 'text-pi-accent bg-pi-accent/10' : 'text-pi-text-dim')}
          ...
          <button
            ...
            className={cn('p-0.5 rounded', useRegex ? 'text-pi-accent bg-pi-accent/10' : 'text-pi-text-dim')}
            title="Use regex"
          >
        ...
        <span className="text-[10px] text-pi-text-dim whitespace-nowrap">
          {results.length > 0 ? `${selectedIdx + 1}/${results.length}` : 'No results'}
        </span>

        <button onClick={() => setSelectedIdx(prev => Math.max(0, prev - 1))} className="p-1 text-pi-text-dim hover:text-pi-text">
          <ChevronUp className="w-3.5 h-3.5" />
        </button>
        <button onClick={() => setSelectedIdx(prev => Math.min(results.length - 1, prev + 1))} className="p-1 text-pi-text-dim hover:text-pi-text">
          <ChevronDown className="w-3.5 h-3.5" />
        </button>
        <button onClick={() => setShowReplace(!showReplace)} className="p-1 text-pi-text-dim hover:text-pi-text" title="Toggle replace">
          <Replace className="w-3.5 h-3.5" />
        </button>
        <button onClick={onClose} className="p-1 text-pi-text-dim hover:text-pi-text">
      ...
      {showReplace && (
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center gap-1.5 bg-pi-surface border border-pi-border rounded-lg px-2">
            <Replace className="w-3.5 h-3.5 text-pi-text-dim shrink-0" />
            <input
              value={replaceTerm}
          ...
          <button
            ...
            className="px-2 py-1 text-[10px] bg-pi-surface border border-pi-border rounded hover:bg-pi-accent/10 text-pi-text-dim hover:text-pi-text disabled:opacity-50 transition-colors"
          >
            Replace
          </button>
          <button
