...
function getServiceRoleKey(): string | null {
  const key =
    process.env.CHEAPSKIP_SUPABASE_SERVICE_ROLE_KEY ||
...
export async function getLeadStats(): Promise<LeadStats> {
  ...
  const [rToday, rWeek, rMonth] = await Promise.all([
    cheapskipFetch('quote_requests', `select=id&created_at=gte.${today}&status=neq.test`),
    cheapskipFetch('quote_requests', `select=id&created_at=gte.${weekAgo}&status=neq.test`),
    cheapskipFetch('quote_requests', `select=id&created_at=gte.${monthAgo}&status=neq.test`),
  ])
...
/** Total and active skip providers */
export async function getProviderCount(): Promise<ProviderCount> {
  const [rTotal, rActive] = await Promise.all([
    ...
    cheapskipFetch('skip_providers', 'select=id&status=eq.active'),
  ])
...
/** Revenue from accepted lead_distributions, grouped by period */
export async function getRevenueSnapshot(): Promise<RevenueSnapshot> {
  ...
  const [rToday, rWeek, rMonth] = await Promise.all([
    cheapskipFetch('lead_distributions', `select=id&status=eq.accepted&accepted_at=gte.${today}`),
    cheapskipFetch('lead_distributions', `select=id&status=eq.accepted&accepted_at=gte.${weekAgo}`),
    cheapskipFetch('lead_distributions', `select=id&status=eq.accepted&accepted_at=gte.${monthAgo}`),
  ])
  ...
  return {
    today: countFrom(rToday) * LEAD_PRICE_AUD,
    thisWeek: countFrom(rWeek) * LEAD_PRICE_AUD,
    thisMonth: countFrom(rMonth) * LEAD_PRICE_AUD,
  }
}
...
export async function getLeadSummary(): Promise<string> {
  try {
    const [stats, providers, revenue] = await Promise.all([
      getLeadStats(),
      getProviderCount(),
      getRevenueSnapshot(),
    ])
    return `Leads: ${stats.today} today / ${stats.thisWeek} week / ${stats.thisMonth} month | Revenue: $${revenue.thisMonth} | Providers: ${providers.active} active`
  } catch (err) {
