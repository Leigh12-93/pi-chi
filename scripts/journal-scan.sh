...
journalctl -u pi-chi-brain -n 200 --no-pager -o short-iso 2>/dev/null \
  ...
  | grep -vE "0 errors|[Zz]ero errors|crash counter|Crash counter|[Ww]ake cycle|counter: 0|counter:0" \
  ...
  | grep -vE "[Zz]ero errors in last|Everything is clean|everything is clean" \
